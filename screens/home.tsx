...
export default function Home({ navigation: { navigate } }: any) {
  ...
  const {
    queryRoom: { data, isLoading, isRefetching },
    calendarViewType,
    isVisibleFullCalendar,
    activeBookId,
    changeActiveBookId,
  } = useCalendar();
  const { mutate: mutateBookPos, isPending: changingBookPos } =
    useChangePosBookMutate();
  ...
  const onStateChange = () =>
    setIsModalState((prev) =>
      prev === Modal_States.fab ? null : Modal_States.fab
    );
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <Suspense
        fallback={
          <ActivityIndicator animating={true} color={colors.menuColor} />
        }
      >
        <WeekCalendar
          date={date}
            ...
            <ScrollView style={{ marginBottom: 10 }}>
              <RadioButton.Group
                onValueChange={(newValue) =>
                  changeActiveBookId((prev) => ({ ...prev, roomId: newValue }))
                }
                value={String(activeBookId?.roomId)}
              >
      ...
      {!isLoading && (
        <Portal>
          <FAB.Group
            ...
            visible={
              isFocused &&
              (isModalState === null || isModalState === Modal_States.fab)
            }
            icon={"plus"}
