...
const twentyElements = Array.from({ length: 21 }, (_, index) => ({
  label: index + "",
  value: index,
}));
...
export default function EventForm({
  ...
  const { values, setFieldValue, resetForm, handleChange, handleSubmit } =
    useFormik({
      initialValues,
      onSubmit,
    });
  ...
  const { data: roomsNames, isLoading: isLoadingRoomsNames } =
    useGetQueryRoomsNames({ mode: mode || "" });
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (mode === "create" && start_date !== undefined) {
        ...
        const startDateInTargetZone = format(
          utcToZonedTime(selectedDate, targetTimeZone),
          "yyyy-MM-dd"
        );
      ...
      const data =
        typeof eventData === "string" ? JSON.parse(eventData) : undefined;
        ...
        <View style={{ flexDirection: "row" }}>
          ...
          {mode === "update" && (
            <IconButton
              ...
              iconColor={colors.surface}
              onPress={() => {
                Alert.alert("Ви дійсно хочете видалити бронювання?", "", [
                  {
                    text: "Ні", // Button text
                  },
                  {
  ...
  useEffect(() => {
    if (values.start_date && values.end_date) {
      if (values.price_per_day || values.final_price) {
        ...
        setFieldValue("payment_on_place", finalPrice - values.down_payment);
      }
    }
      ...
      >
        ...
        >
          ...
          <DatePickerInput
            ...
            defaultValue={
              values.start_date &&
              (new Date(values.start_date).getDate() + 1).toString()
            }
          />
          <View style={{ marginTop: 15, flex: 1, alignItems: "center" }}>
            <Button
              ...
              onPress={() => onShow(pickModalTypes.TA)}
              uppercase={false}
              mode="outlined"
            >
              Час при'їзду{" "}
              {values.time_arrival
                ? values.time_arrival?.toString()
                : "(невказано)"}
            </Button>
            <TimePickerModal
          ...
          <View style={{ marginTop: 15, flex: 1, alignItems: "center" }}>
            <Button
              ...
              textColor={colors.surface}
              buttonColor={colors.orangeColor}
              onPress={() => onShow(pickModalTypes.TD)}
              uppercase={false}
              mode="outlined"
            >
              Час від'їзду{" "}
              {values.time_departure
                ? values.time_departure?.toString()
                : "(невказано)"}
            </Button>
            <TimePickerModal
            ...
            onPress={(type) => {
              if (values.start_date) {
                if (values.end_date) {
                  setFieldValue(
                    "end_date",
                    addDays(new Date(values.end_date), type === "plus" ? 1 : -1)
                  );
                } else {
                  setFieldValue(
                    "end_date",
                ...
                Alert.alert("Виберіть стартову дату!", "", [{ text: "OK" }]);
              }
            }}
          />
        </Surface>
        ...
        >
          ...
          {(isLoading || isLoadingCreateSoruce) && (
            <ActivityIndicator animating />
          )}
          <View style={{ position: "relative" }}>
            {!isLoading &&
              !isLoadingCreateSoruce &&
              data &&
              data?.length > 0 && (
                <>
                  <View>
                    <Menu
                      ...
                      anchor={
                        <Button
                          ...
                          mode="outlined"
                          onPress={() => setDropsListState(dropStates.source)}
                        >
                          {values.sources_id !== 0
                            ? data.find(
                                ({ value }) =>
                                  Number(value) === values.sources_id
                              )?.label
                            : "Походження:"}
                        </Button>
                      }
                  ...
                  >
                    ...
                    <TouchableOpacity
                      onPress={() => setFieldValue("sources_id", 0)}
                    >
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              if (values.start_date && values.end_date) {
                ...
                setFieldValue(
                  "payment_on_place",
                  final_price - (values.down_payment ?? 0)
                );
              }
              setFieldValue("price_per_day", Number(text));
            }}
          ...
          <TextInput
            ...
            onChangeText={(number) => {
              ...
              if (values.start_date && values.end_date) {
                ...
                setFieldValue(
                  "price_per_day",
                  formatted_number / daysDifference
                );
                setFieldValue(
                  "payment_on_place",
                  formatted_number - (values.down_payment ?? 0)
                );
              }
              setFieldValue("final_price", formatted_number);
            }}
            keyboardType="numeric"
          />
          <TextInput
            ...
            onChangeText={(number) => {
              if (values.final_price) {
                setFieldValue(
                  "payment_on_place",
                  Math.abs(Number(values.final_price) - Number(number))
                );
              }
              setFieldValue("down_payment", Number(number));
            }}
            keyboardType="numeric"
          />
          <DatePickerInput
